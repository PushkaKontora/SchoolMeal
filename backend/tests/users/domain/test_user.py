...
from app.users.domain.user import NotVerifiedPasswordError, User
...
def test_authentication_using_incorrect_password(user: User):
    with pytest.raises(NotVerifiedPasswordError):
...
def user(last_name: LastName, first_name: FirstName, phone: Phone, email: Email, password: Password) -> User:
    return User.create_parent(first_name, last_name, phone, email, password)
