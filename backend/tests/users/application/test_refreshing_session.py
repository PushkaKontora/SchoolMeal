...
async def test_refreshing(refresh_token: str, users_service: UsersService):
    access_token, refresh_token = await users_service.refresh_session(refresh_token)

    validate_tokens(access_token, refresh_token, expected_user_id=refresh_token.user_id)


async def test_refreshing_after_logout(access_token: str, refresh_token: str, users_service: UsersService):
    await users_service.logout(access_token)

    with pytest.raises(SessionIsAlreadyRevoked):
        await users_service.refresh_session(refresh_token)


async def test_refreshing_using_revoked_token(refresh_token: str, users_service: UsersService):
    await users_service.refresh_session(refresh_token)
