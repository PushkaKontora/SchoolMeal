...
class GetSchoolClassesQueryExecutor(IQueryExecutor[GetSchoolClassesQuery, list[SchoolClass]]):
    def __init__(self, session_factory: Callable[[], AsyncSession]) -> None:
        self._session_factory = session_factory

    async def execute(self, query: GetSchoolClassesQuery) -> list[SchoolClass]:
        async with self._session_factory() as session:
            ...
            return [SchoolClass.from_db(school_class_db) for school_class_db in school_classes_db]
