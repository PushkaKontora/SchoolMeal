...
class MealService:
    @inject
    async def get_meals(self, options: MealsOptions, uow: UnitOfWork = Provide[Database.unit_of_work]) -> list[MealOut]:
        async with uow:
            spec = (
        ...
        return [self._get_meal_out(meal) for meal in meals]
